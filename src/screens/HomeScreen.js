...
import { StyleSheet, Text, View,FlatList,SafeAreaView,Image ,TextInput,Dimensions,TouchableOpacity} from 'react-native';
...
export default function HomeScreen({navigation}) {
    const renderRecipeItem =({item,index})=>{
        return(
            <Card customStyle={{marginBottom:15,padding:10}}>
    ...
    const renderRecipeItemcols =({item,index})=>{
        return(
            <Card customStyle={{flex:1,margin:4}}
            >
            <Image source={{uri:item.imageURL}}
                style={{
                    height:200,
            ...
            />
            <View style={{padding:20}}>
            <Text numberOfLines={1} style={{fontSize:14,fontWeight:'600'}}>{item.name}</Text>
            </View>
    ...
    const renderRecipeItemHorizontal =({item,index})=>{
        return(
            <Card customStyle={{width:width*0.5,margin:5}}
            >
            <Image source={{uri:item.imageURL}}
                style={{
                    height:200,
